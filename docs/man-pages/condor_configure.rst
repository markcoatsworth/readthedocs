 ...
 **—install[=<path/to/release>]**
    ...
    configuration files and local directory are preserved. This is the
    default behavior of *condor\_install*.
 **—install-dir=<path>**
 ...
 **—local-dir=<path>**
    ...
    location is indicated by the LOCAL\_DIR variable in the
    ...
    the new directory will be created and the log, spool and execute
 ...
 **—central-manager=<hostname>**
    ...
    variable COLLECTOR\_HOST to point to the given host name. The
 ...
 **—owner=<ownername>**
    ...
    execute directories and sets the CONDOR\_IDS value in the
 ...
 **—overwrite**
    ...
    *condor\_install* what to do.
    ...
    This prevents *condor\_install* from moving an sbin directory out of
    ...
    /usr/local, etc.) For example: *condor\_install* **–prefix=/usr**
    will not move /usr/sbin out of the way unless you specify the
    **–backup** option.
 ...
 **—backup**
    ...
    default, *condor\_install* will not install if it finds an existing
    sbin directory with HTCondor programs in it. In this case,
    *condor\_install* with exit with an error message. You must specify
    **–overwrite** or **–backup** to tell *condor\_install* what to do.
    ...
    **–prefix=/usr** will not move /usr/sbin out of the way unless you
 ...
 **—no-env-scripts**
    ...
    user’s PATH and CONDOR\_CONFIG environment variables. This option
    prevents *condor\_configure* from generating these scripts.
...
To change the location of the LOCAL\_DIR directory in the configuration
